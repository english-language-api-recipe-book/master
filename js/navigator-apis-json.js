...
function APIJSONNavigatorGetHeader(name,description,url,image,apijsonurl)
    ...
    html = html + '<a href="' + url + '" title="' + name + '"><img src="' + image + '" width="175" align="left" style="padding: 20px;" /></a>';
...
function APIJSONNavigatorGetAPIListing(name,description,image,url,$apicount)
    ...
    html = html + '<a href="' + url + '" style="color: #000; font-size: 18px; text-decoration: none;" title="' + name[0].toUpperCase() + name.slice(1) + '" target="_blank"><strong>' + name[0].toUpperCase() + name.slice(1) + '</strong></a>';
    html = html + '</td>';

		html = html + '<td align="center" style="padding-top: 5px; padding-bottom: 5px;" colspan="2">';
    html = html + '<a href="' + url + '" style="color: #000; font-size: 18px; text-decoration: none;" title="' + name[0].toUpperCase() + name.slice(1) + '" target="_blank"><strong>{"OADF JSON"}</strong></a>';
    html = html + '</td>';
